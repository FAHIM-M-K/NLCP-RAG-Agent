...
# from langchain_mcp_adapters.client import MultiServerMCPClient # Remove if not used with StdioServerParameters directly
# from mcp.client.stdio import StdioServerParameters # Remove if not using StdioServerParameters directly


# Re-import just MultiServerMCPClient if StdioServerParameters is no longer directly used in the constructor
# (if the library expects direct dicts and we need to pass env as part of it)
from langchain_mcp_adapters.client import MultiServerMCPClient 
...
async def initialize_rag_agent_with_mcp():
    ...
    # *** REVERTING TO DIRECT DICTIONARY CONFIGURATION, AND ADDING ALL KEYS ***
    # This assumes MultiServerMCPClient expects a dictionary where each value
    # is another dictionary of server parameters, including 'transport' and 'env'.
    mcp_client = MultiServerMCPClient(
        {
            "mongodb": {
                "transport": "stdio",
                "command": "python",
                "args": ["mongodb_tools.py"],
                "env": os.environ.copy() # <-- Pass environment variables as a dictionary directly
            },
            "mysql": {
                "transport": "stdio",
                "command": "python",
